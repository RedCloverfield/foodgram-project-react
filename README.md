...
![Foodgram CI](https://github.com/redcloverfield/foodgram-project-react/actions/workflows/main.yml/badge.svg) \
...
* ### Разворачиваем используя [Github](https://github.com/)
    ...
    ```bash 
    git clone git@github.com:redcloverfield/foodgram-project-react.git
    ```
    ...
    ```bash
    sudo docker compose -f [имя-файла-docker-compose.yml] exec backend python manage.py load_ingredients_data
    ```
...
## Документация API проекта
  ...
  Пример: **https://foodgram.myddns.me/api/docs/** (кликабельно, можно ознакомиться)
